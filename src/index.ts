...
server.tool = function (name, description, schema, handler) {
  log.info(`Registering tool with original parameters: ${name}`, {
    schema:
      typeof schema === "object" ? JSON.stringify(schema) : "non-object schema",
  });
...
if ((server as any)._tools) {
  ...
  log.info("Server has _tools property", {
    count: Object.keys(originalTools).length,
  });
}
if ((server as any)._toolSchemas) {
  originalToolSchemas = (server as any)._toolSchemas;
  log.info("Server has _toolSchemas property", {
    count: Object.keys(originalToolSchemas).length,
  });
}
...
async function registerTools() {
    ...
    try {
      ...
      log.info("Using formatted schema", {
        ...
        transformedSchema: transformedSchema
          ? JSON.stringify(transformedSchema).substring(0, 100) + "..."
          : "null",
      });
        ...
        server.tool(
          ...
          async (params: any) => {
            try {
              ...
              log.error("Error in tool handler", {
                title: formattedTitle,
                error,
              });
  ...
  writeToLogFile("registered_tools.log", {
    ...
    totalTools: (server as any)._tools
      ? Object.keys((server as any)._tools).length
      : 0,
  });
}
          ...
          if (
            ...
            if (Array.isArray(tools) && tools.length > 0) {
              log.info(`OUTGOING STDIO MESSAGE WITH TOOLS (${tools.length})`, {
                ...
                schemaProperties: tools[0].inputSchema
...
function cleanAllRegisteredTools() {
  ...
  log.info(
    "cleanAllRegisteredTools is disabled to avoid breaking client connections"
  );
  return;
}
...
function patchRegisterTool() {
  ...
  log.info(
    "patchRegisterTool is disabled to avoid breaking client connections"
  );
  return;
}
  ...
  try {
    ...
    if ((server as any)._toolSchemas) {
      log.info("Final tool schemas before connecting", {
        ...
        schemaList: Object.entries((server as any)._toolSchemas).map(
          ([name, schema]) => ({
            name,
...
export const startMCP = main;

// Only run automatically when the script is executed directly (not imported)
...
function transformResponseFormat(response: any, method: string): any {
  ...
  log.info(`Response transformed to JSON-RPC 2.0 format`, {
    ...
    sampleTool:
      formattedResponse.result.tools.length > 0
